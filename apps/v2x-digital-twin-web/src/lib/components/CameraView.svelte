...
<div class="relative w-full h-full bg-black">
	...
	<!-- Camera view toggle buttons - bottom center -->
	<div class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1 bg-black/60 rounded-lg p-1 pointer-events-auto">
		{#each CAMERA_VIEWS as view}
			<button
				onclick={() => onSwitchView(view.id as CameraView)}
				class="px-3 py-1.5 text-xs rounded-md transition-colors {activeView === view.id
					? 'bg-white/20 text-white'
					: 'text-gray-400 hover:text-white hover:bg-white/10'}"
			>
				{view.label}
				<span class="ml-1 text-gray-500">{view.key}</span>
			</button>
		{/each}
	</div>
</div>
